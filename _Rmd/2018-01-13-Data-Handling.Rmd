...
author: "ooDoo"
date: 2018-01-08
image: /assets/article_images/title/siba.jpg
---
...
##2) lapply
```{r}
lapply(iris[ ,1:4],mean) #결과물을 dataFram으로 변환 용이
```
...
#install.packages('nycflights13') # 예제 데이터 
...
filter(flights, month ==1 , dep_time >= 1630)
# 조건이 2개 이상도 그냥 ','를 사용해서 추가 시켜 줄 수 있음!
```

##2) dplyr: `select`
 : 원하는 컬럼을 선택하는 함수
```{r}
select(flights, dep_time, arr_time) # dep_time, arr_time 두개의 컬럼을 뽑아냄
select(flights, month:arr_delay) # 여러개의 컬럼을 이렇게 연속해서 불러 낼 수있음, 이때 컬럼 번호를 몰라도 된다는 장점
#time과 관련된 모든 컬럼을 뽑아내고자 할때, using [contains]조건문
select(flights, contains('time'))
#time와 관련된 모든 컬럼을 빼고자 할때 , using [-]조건문
select(flights, -contains('time'))
```
...
arrange(flights, desc(distance)) # distance컬럼을 기준으로 내림차순 정렬
arrange(flights, distance, dep_time) # distance를 먼저 기준으로 정렬 후 , dep_time을 다음 기준으로 정렬
```
...
```{r}
#속력= 거리/시간
flightsSpeed <- mutate(flights, speed = distance /air_time * 60) # 그런데 이때 거리의 단위는 시간당 마일이기에 60을 곱
